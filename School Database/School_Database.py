...
PANEL_RIGHT_COLOR="SlateGray1"
PANEL_LEFT_COLOR="LightSkyBlue3"
BORDER_FRAMERIGHT_COLOR="blue2"

MATH_FRAME_BACKGROUNDCOLOR_HEADER="royal blue"
MATH_FRAME_BACKGROUNDCOLOR="LightSkyBlue3"

SPANISH_FRAME_BACKGROUNDCOLOR="pink"
SPANISH_FRAME_BACKGROUNDCOLOR_HEADER="indian red"

HISTORY_FRAME_BACKGROUNDCOLOR_HEADER="orange"
HISTORY_FRAME_BACKGROUNDCOLOR="khaki"

EXCEL_BUTTON_BACKGROUNDCOLOR="sea green"
CSV_BUTTON_BACKGROUNDCOLOR="aquamarine2"

ID_NAME_EMAIL_AULA_BACKGROUNDCOLOR="gray68"

FONT_BUTTON=("Bahnschrift Condensed",11)
...
def eliminar_ahora(estudiante_o_profesor,id_reference):
    con=sqlite3.connect('Escuela.db')    #Create a database or connect to one
    c= con.cursor()                     #Create cursor
    
    c.execute(f"SELECT nombre_completo FROM {TABLE_NAME[estudiante_o_profesor]} WHERE oid = {str(id_reference)}")

    valor=messagebox.askyesno("Eliminar Registro "+str(id_reference),"¿Está seguro que desea eliminar el registro de la base de datos?")
    if valor==True:
        c.execute(f"DELETE FROM {TABLE_NAME[estudiante_o_profesor]} WHERE oid = {str(id_reference)}")
...
def rellene_todos_los_campos():
    ...
    warnlbl=Label(top,bg=PANEL_RIGHT_COLOR,font=("Bahnschrift Condensed",14), text="Por favor rellene todos los campos",pady=20,padx=20)
    warnlbl.pack()
    return 
...
def aula_sin_estudiantes():
    ...
    top.iconbitmap("logo1.ico")
    top.geometry("300x100")
    warnlbl=Label(top,bg=PANEL_RIGHT_COLOR,font=("Bahnschrift Condensed",14), text="En el grupo actual no hay estudiantes.\n Asigne al profesor a otro grupo",pady=20,padx=20)
    warnlbl.pack()
    f_aula.delete(0,END)
...
def auth():
    if e_user.get()=="Admin" and e_password.get()=="a":
        is_admin()
        return
    else:
...
def login():
    ...
    l_frame_user_auth=LabelFrame(root,width=275,height=350,background=PANEL_LEFT_COLOR,bd=3,font=("Bahnschrift Condensed",12))
    l_frame_user_auth.pack(side=LEFT,fill=Y)

    global mylogo
    mylogo=ImageTk.PhotoImage(Image.open("logo1.png").resize((100,100)))
    mylabel=Label(l_frame_user_auth,image=mylogo,borderwidth=0,bg=PANEL_LEFT_COLOR)
    mylabel.pack(pady=80,padx=50)
    
    #anchoFLeft=500-l_frame_user_auth.__getitem__("width")
    global RightFrame
    RightFrame=LabelFrame(root,width=225,height=700,background=PANEL_RIGHT_COLOR,text="Iniciar Sesión",bd=3,font=("Bahnschrift Condensed",12))
    RightFrame.pack(side=RIGHT,fill=BOTH,expand=1)

    global l_user
    l_user=Label(RightFrame,text="Name",bg=PANEL_RIGHT_COLOR,font=("Bahnschrift Condensed",15))
    ...
    global l_password
    l_user=Label(RightFrame,text="Password",bg=PANEL_RIGHT_COLOR,font=("Bahnschrift Condensed",15))
    ...
    check_user_btn=Button(RightFrame,image=mybtnloginimg,command=auth,cursor="hand2",borderwidth=0,bg=PANEL_RIGHT_COLOR,activebackground=PANEL_RIGHT_COLOR)
